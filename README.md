...
- **`/sync-upstream`** syncs this fork with the upstream template. Fork-specific: beyond resolving the usual rebase conflicts, it hunts the changes that merge *cleanly and are still wrong* here — orphaned files left in directories this fork deleted, CI matrices and docs naming removed portals, Danish locale defaults reintroduced into skill prose, and upstream hardening that cannot reach the Spanish CLIs because they are independent implementations. Ends with typecheck, tests, guards, and a live probe per portal before pushing.
