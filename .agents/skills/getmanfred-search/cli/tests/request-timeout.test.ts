...
import { fetchText, REQUEST_TIMEOUT_MS } from "../src/helpers";
...
describe("request timeout", () => {
  test("fetchText passes an AbortSignal timeout to fetch", async () => {
    ...
    await fetchText("https://www.getmanfred.com/x", "application/json");
  ...
  test("a timeout surfaces immediately and is not swallowed by the retry loop", async () => {
    ...
    await expect(fetchText("https://www.getmanfred.com/x", "application/json")).rejects.toThrow(/timed out/);
    expect(calls).toBe(1);
  });
