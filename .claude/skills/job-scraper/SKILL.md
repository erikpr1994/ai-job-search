...
3. Scope to the last 14 days using the portal's supported recency **filter** flag (`--jobage`, `--since <YYYY-MM-DD>`, etc. — as documented per portal). A portal with **no recency flag**, or one that only offers a coarse native window (Tecnoempleo's native filter is a 24h bucket; `--jobage 7/14/30` filters within the fetched page rather than server-side, and GetManfred implements `--jobage` client-side entirely), still gets scoped: every portal's search output carries a `date` field, so filter client-side — drop results whose `date` is older than 14 days after the call returns, and never invent a flag the portal's SKILL.md does not document (the CLIs reject unknown flags). `--order PublicationDate` is a sort, and a sort is not a filter — pairing it with a `--limit` is a defensible approximation on a portal that offers nothing better, but apply the client-side date filter on top all the same.
